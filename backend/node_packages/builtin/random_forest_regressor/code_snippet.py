"""Code snippet for Random Forest Regressor node."""
...
def get_snippet(params: dict, input_vars: dict, output_vars: dict, label: str):
    ...
    feat_arg = f'max_features="{max_features}"' if isinstance(max_features, str) else f"max_features={max_features}"

    code = (
        f'X_train = {split}_train.drop(columns=[{split}_target_col])\n'
        f'y_train = {split}_train[{split}_target_col]\n'
        f'{df_out} = {split}_test\n'
        f'{model_out} = RandomForestRegressor(n_estimators={n_estimators}, {depth_arg}, {feat_arg}, random_state={random_state})\n'
